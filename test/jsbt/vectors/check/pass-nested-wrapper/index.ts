...
type TRet<T> = T &
  (T extends (...args: infer A) => infer R
    ? ((...args: { [K in keyof A]: TArg<A[K]> }) => TRet<R>) & { [K in keyof T]: T[K] }
    : T);
type OutputOpts = { dkLen?: number };
...
type WebHash = TRet<HashInstance> & { isSupported: () => Promise<boolean> };
/**
 * Web hash helper.
 ...
 * ```
 */
export const sha: TRet<WebHash> = Object.assign(
  (msg: Uint8Array, opts?: OutputOpts) => Uint8Array.from(msg).slice(0, opts?.dkLen || msg.length),
