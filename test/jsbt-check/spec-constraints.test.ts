...
// Suite directories and their aggregators: `npm test` (test/index.ts) runs test/jsbt
// and test/bench; `npm run test:check` (test/check.ts) runs test/jsbt-check.
...
const INDEX_DIRS = ['test/jsbt', 'test/bench'] as const;
...
const jsbtImport = /import\s+\{([^;]+?)\}\s+from\s+'(\.\.\/\.\.\/src\/(?:jsbt\.ts|jsbt-check\/[^']+))'/g;
...
should('constraint: npm run check size audits without printing size stats', () => {
  ...
  passes(res);
  // Size stats are `jsbt size`'s job; the check runs the audit and reports only that.
