...
const ROOT = resolve('.');
const NPM_FIXTURE = join(ROOT, 'test/jsbt-check/vectors/npm-check');
const npmEnv = {
  ...process.env,
  JSBT_FAST: '',
  JSBT_QUIET: '',
...
// `jsdoc` is a legacy alias for `tsdoc`: documented in usage, kept out of README.
const readmeSelectors = checkSelectors.filter((selector) => selector !== 'jsdoc');
// Suite directories and their aggregators: `npm test` (test/index.ts) runs
// test/benchmark; `npm run test:check` (test/check.ts) runs test/jsbt-check.
// The `baler` binary's suite lives in baler/test and runs via its own package.
// test/test hosts standalone runner self-tests (they call should.run() themselves,
// deliberately fail, or mutate global opts) and belongs to no aggregated suite.
const INDEX_DIRS = ['test/benchmark'] as const;
const CHECK_DIRS = ['test/jsbt-check'] as const;
const SUITE_DIRS = [...INDEX_DIRS, ...CHECK_DIRS];
const testsIn = (dir: string) =>
  sorted(readdirSync(join(ROOT, dir)).filter((file) => file.endsWith('.test.ts')));
const dirTests = (dir: string) => testsIn(dir).map((file) => `${dir}/${file}`);
const suiteTests = () => SUITE_DIRS.flatMap(dirTests);
...
const testTsFiles = (): string[] => sorted(SUITE_DIRS.flatMap((dir) => walkTs(dir)));
const jsbtImport = /import\s+\{([^;]+?)\}\s+from\s+'(\.\.\/\.\.\/src\/jsbt-check\/[^']+)'/g;
...
should('test/index.ts and test/check.ts split every jsbt test file between them', () => {
  const importsOf = (file: string, dir: string) => {
    const sub = dir.replace(/^test\//, '');
    return Array.from(
      read(file).matchAll(new RegExp(`import '\\./${sub}/([^']+\\.test\\.ts)'`, 'g'))
    ).map((item) => `${dir}/${item[1]}`);
  };
  // Disjoint: each aggregator imports only from its own suite directories, and the
  // standalone runner self-tests in test/test never join an aggregated suite.
  for (const dir of [...CHECK_DIRS, 'test/test'])
    deepStrictEqual(importsOf('test/index.ts', dir), []);
  for (const dir of [...INDEX_DIRS, 'test/test'])
    deepStrictEqual(importsOf('test/check.ts', dir), []);
  // Complete: each aggregator covers every test file in its directories.
  const index = INDEX_DIRS.flatMap((dir) => importsOf('test/index.ts', dir));
  const check = CHECK_DIRS.flatMap((dir) => importsOf('test/check.ts', dir));
  deepStrictEqual(sorted(index), sorted(INDEX_DIRS.flatMap(dirTests)));
  deepStrictEqual(sorted(check), sorted(CHECK_DIRS.flatMap(dirTests)));
  // `npm test` covers only the jsbt binary and shared modules; check code stays out.
  const bad = index.filter((file) => read(file).includes('src/jsbt-check/'));
  deepStrictEqual(bad, []);
});

should('every standalone jsbt test file opts into runWhen', () => {
  const missing = suiteTests().filter(
    (file) => !read(file).includes('should.runWhen(import.meta.url)')
  );
...
should('jsbt files with fs-modify constraints do not import raw destructive fs helpers', () => {
  const files = [
    'baler/src/baler.ts',
    'baler/src/fs-modify.ts',
    ...readdirSync(join(ROOT, 'src/jsbt-check'))
      .filter((file) => file.endsWith('.ts'))
      .map((file) => `src/jsbt-check/${file}`),
  ];
  const bad: string[] = [];
...
should('jsbt check exposes only documented selectors', () => {
  const src = read('src/jsbt-check/check.ts');
...
should('README and CLI usage document every check selector', () => {
  ...
  const usage = read('src/jsbt-check/check.ts');
  const missing: string[] = [];
  for (const selector of checkSelectors) {
    const jsbt = `jsbt-check ${selector}`;
    const npm = `npm run check ${selector}`;
    if (!usage.includes(jsbt)) missing.push(`src/jsbt-check/check.ts: ${jsbt}`);
...
should('constraint: npm run check size audits without printing size stats', () => {
  const res = runNpmCheck(['size']);
  passes(res);
  // Size stats are `baler size`'s job; the check runs the audit and reports only that.
  deepStrictEqual(/module,export,loc|@jsbt-test\/npm-check,,/.test(res.text), false, res.text);
  has(res.text, /1 check finished in \d+ sec/);
});
...
should('constraint: npm run check locks output contracts for every other selector', () => {
  const contracts = {
    ...
    bytes: {
      pass: true,
      has: [
        // bytes-helper guidance is suppressed for now; wrap findings still report.
