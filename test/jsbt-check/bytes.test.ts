...
import { bytesVector } from './bytes-vectors.ts';

const should = Object.assign(test.serial, { runWhen: test.runWhen });
const ROOT = resolve('test/jsbt-check/vectors/bytes');
const BUILD = resolve('test/jsbt-check/build/bytes-boundary');
const POLARITY = bytesVector('polarity');
const POLARITY_BUILD = resolve('test/jsbt-check/build/bytes-polarity');
...
const { runCli: runBytes } = await import('../../src/jsbt-check/bytes.ts');
...
should('bytes handles recursive mapped generic helper types', async () => {
  const cwd = bytesVector('cycle');
  const res = await capture(() =>
    runBytes(['package.json'], { color: false, cwd, loadTS: () => ts })
  );
  deepStrictEqual(res.ok, false, all(res));
  // bytes-helper guidance is suppressed for now; only the wrap findings report.
...
should('bytes suppresses canonical helper guidance on missing helpers', async () => {
  const cwd = bytesVector('helper-missing');
  const res = await capture(() =>
    runBytes(['package.json'], { color: false, cwd, loadTS: () => ts })
  );
...
should('bytes suppresses canonical helper guidance on outdated helper docs', async () => {
  const cwd = bytesVector('helper-old');
  const res = await capture(() =>
    runBytes(['package.json'], { color: false, cwd, loadTS: () => load('6.0.2') })
  );
  deepStrictEqual(res.ok, false, all(res));
...
should('bytes accepts canonical TArg/TRet re-export helpers', async () => {
  const cwd = bytesVector('helper-reexport');
  const res = await capture(() =>
    runBytes(['package.json'], { color: false, cwd, loadTS: () => load('6.0.2') })
  );
  deepStrictEqual(res.ok, false, all(res));
...
should('bytes suggests mechanical TArg/TRet wrappers at API boundaries', async () => {
  ...
  deepStrictEqual(/summary: 0 passed, 0 warnings, 6 failures, 0 skipped/.test(plain(res)), true);
});

should('bytes skips unsafe whole-object wrappers around opaque domain objects', async () => {
  const cwd = bytesVector('opaque');
  const res = await capture(() =>
    runBytes(['package.json'], { color: false, cwd, loadTS: () => ts })
  );
  const out = plain(res);
  ...
  deepStrictEqual(/summary: 0 passed, 0 warnings, 6 failures, 0 skipped/.test(out), true);
});
...
should('bytes reports input and output typed-array issues on bytes fixture', async () => {
  ...
  deepStrictEqual(/summary: 8 passed, 0 warnings, 59 failures, 0 skipped/.test(all(res)), true);
