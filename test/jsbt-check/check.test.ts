...
const JSBT_ENTRY = join(BASE, 'src/jsbt-check/check.ts');
...
const { runCli: runTSDoc } = await import('../../src/jsbt-check/jsdoc.ts');
const { runCli: runCheckBin } = await import('../../src/jsbt-check/check.ts');
// Legacy shim over the jsbt-check binary: strips the old `check` command head.
// The `jsbt` binary's size/bundle commands are covered in size.test.ts and bundle.test.ts.
const runJsbt = (argv: string[], opts: Record<string, unknown> = {}) =>
  runCheckBin(argv[0] === 'check' ? argv.slice(1) : argv, opts);
const { runCli: runBigInt } = await import('../../src/jsbt-check/bigint.ts');
const { runCli: runComments } = await import('../../src/jsbt-check/comments.ts');
const { runCli: runImportTime } = await import('../../src/jsbt-check/importtime.ts');
const { docExampleImportNames, runCli: runReadme } = await import('../../src/jsbt-check/readme.ts');
const { runCli: runTypeImport } = await import('../../src/jsbt-check/typeimport.ts');
const { runSizeCheckCli: runTreeshake } = await import('../../src/jsbt-check/check.ts');
const { wantColor } = await import('baler/env.js');
...
const typeImportProof = () => {
  const root = resolve('test/jsbt-check/build/typeimport-proof');
...
should('readme runs examples importing allowlisted third-party deps', async () => {
...
// Standalone treeshake runs need a jsbt-owned temp out dir; nothing may land in the repo.
const withTreeshakeTmp = async (
  fn: (outDir: string) => Promise<void>
): Promise<ReturnType<typeof capture>> => {
  const tmp = mkdtempSync(join(tmpdir(), 'baler-check-ts-'));
  try {
    return await capture(() => fn(join(tmp, 'out')));
  } finally {
...
should('treeshake passes on root-entry fixture', async () => {
  const cwd = fixture('pass-root');
  const res = await withTreeshakeTmp((outDir) => runTreeshake({ cwd, outDir }));
  deepStrictEqual(res.ok, true, all(res));
...
should('size check passes bundles within sizeLimits budgets', async () => {
...
should('size check reports bundles over sizeLimits budgets', async () => {
...
should('readme and size --generate-jsbtrc produce and update .jsbtrc.json', async () => {
  const root = resolve('test/jsbt-check/build/genrc');
  rmSync(root, { force: true, recursive: true });
  ...
  try {
    // Each selector generates only its own section.
    const deps = await run(cwd, () => runJsbt(['readme', '--generate-jsbtrc'], { cwd }));
    deepStrictEqual(deps.ok, true, all(deps));
    // Runtime deps and the package itself stay implicit; only extras get pinned.
    deepStrictEqual(readRc(), { exampleDependencies: { '@jsbt-test/dep': '1.2.3' } });
    const sizes = await run(cwd, () => runJsbt(['size', '--generate-jsbtrc'], { cwd }));
    deepStrictEqual(sizes.ok, true, all(sizes));
    const rc = readRc();
    deepStrictEqual(rc.exampleDependencies, { '@jsbt-test/dep': '1.2.3' });
    ...
    const size = await withTreeshakeTmp((outDir) => runTreeshake({ cwd, outDir }));
    deepStrictEqual(size.ok, true, all(size));
    // Regeneration keeps hand-set budgets, refreshes dependency pins, and never
    // touches the other selector's section.
    writeFileSync(
    ...
    const second = await run(cwd, () => runJsbt(['size', '--generate-jsbtrc'], { cwd }));
    deepStrictEqual(second.ok, true, all(second));
    deepStrictEqual(readRc(), {
      exampleDependencies: { '@jsbt-test/dep': '1.0.0' },
      sizeLimits: { 'index.js': '9kb' },
    });
    const third = await run(cwd, () => runJsbt(['readme', '--generate-jsbtrc'], { cwd }));
    deepStrictEqual(third.ok, true, all(third));
    deepStrictEqual(readRc(), {
      exampleDependencies: { '@jsbt-test/dep': '1.2.3' },
      sizeLimits: { 'index.js': '9kb' },
    });
    // tsdoc compiles the same union (README plus TSDoc @example fences).
    rmSync(join(cwd, '.jsbtrc.json'));
    const fourth = await run(cwd, () => runJsbt(['tsdoc', '--generate-jsbtrc'], { cwd }));
    deepStrictEqual(fourth.ok, true, all(fourth));
    deepStrictEqual(readRc(), { exampleDependencies: { '@jsbt-test/dep': '1.2.3' } });
  } finally {
    rmSync(root, { force: true, recursive: true });
  }
...
should('docExampleImportNames extracts bare imports from TSDoc fences', () => {
  const dts = [
    '/**',
    ' * @example',
    ' * ```js',
    " * import { a } from '@scope/a/sub.js';",
    " * import b from 'b';",
    " * import { c } from './local.js';",
    " * import { h } from 'node:crypto';",
    ' * ```',
    ' */',
    "import type { T } from 'outside-fence';",
    'export declare const x: T;',
  ].join('\n');
  deepStrictEqual(docExampleImportNames(dts), ['@scope/a', 'b']);
});

should('check rejects --generate-jsbtrc outside readme, size, and tsdoc selectors', async () => {
  const cwd = fixture('pass-size-limit');
  const res = await run(cwd, () => runJsbt(['bigint', '--generate-jsbtrc'], { cwd }));
  deepStrictEqual(res.ok, false);
  deepStrictEqual(/requires the readme, size, or tsdoc selector/.test(all(res)), true, all(res));
});
...
should('check shows warnings when JSBT_QUIET is set', async () => {
  ...
  deepStrictEqual(
    new RegExp(`broken/broken\\.js:\\d+/retained \\(broken/broken\\)`).test(out),
    true
  );
...
should('check rejects the removed --project option', async () => {
  const cwd = fixture('pass-root');
  const res = await run(cwd, () => checkJsbt(['check', '--project=.', 'comments'], cwd));
  deepStrictEqual(res.ok, false);
  deepStrictEqual(/unknown check option: --project=/.test(plain(res)), true, plain(res));
});
...
should('check size selector audits without printing size stats', async () => {
  const cwd = fixture('pass-root');
  const res = await run(cwd, () => checkJsbt(['check', 'size'], cwd));
  const out = plain(res);
  deepStrictEqual(res.ok, true, all(res));
  // Size stats are `baler size`'s job; the check runs the audit and reports only that.
  deepStrictEqual(/module,export,loc|LOC,|kb (?:min|gzip)/.test(out), false, out);
  deepStrictEqual(/@jsbt-test\/check-root,,/.test(out), false, out);
...
should('check keeps importtime on the serial lane', () => {
  const src = readFileSync(resolve('src/jsbt-check/check.ts'), 'utf8');
  deepStrictEqual(
...
should('bundled importtime does not run imported subcommands', async () => {
  ...
  const out = resolve('test/jsbt-check/build/.__jsbt-bin-test.mjs');
  rmSync(out, { force: true });
  await build({
    banner: { js: '#!/usr/bin/env node' },
    bundle: true,
    define: { __BALER_BUNDLE__: 'true' },
    entryPoints: ['src/jsbt-check/check.ts'],
    format: 'esm',
    outfile: out,
    platform: 'node',
    target: 'node22',
