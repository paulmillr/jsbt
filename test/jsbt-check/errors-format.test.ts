...
const { runCli } = await import('../../src/jsbt-check/errors.ts');
...
should('errors prints rejected wrong values as an audit table', async () => {
  const cwd = resolve('test/jsbt-check/vectors/errors/group-format');
...
should('errors does not require generic value labels in expected/got messages', async () => {
  const cwd = resolve('test/jsbt-check/vectors/errors/value-label');
  const res = await capture(() => runCli(['package.json'], { color: false, cwd }));
  deepStrictEqual(res, {
    ok: true,
    stderr: '',
...
should('errors shows accepted probes for wrong runtime types', async () => {
  const cwd = resolve('test/jsbt-check/vectors/errors/accepted-probes');
  const res = await capture(() => runCli(['package.json'], { color: false, cwd }));
  deepStrictEqual(res, {
    ok: false,
...
should('errors does not infer nested field contracts for generic object helpers', async () => {
  const cwd = resolve('test/jsbt-check/vectors/errors/generic-object');
  const res = await capture(() => runCli(['package.json'], { color: false, cwd }));
  deepStrictEqual(res, {
    ok: true,
    stderr: '',
...
should('errors warns before audit when TSDoc examples do not produce probes', async () => {
  const cwd = resolve('test/jsbt-check/vectors/errors/no-calls');
  const res = await capture(() => runCli(['package.json'], { color: false, cwd }));
  deepStrictEqual(res, {
    ok: true,
    stderr:
...
should('errors does not warn for zero-argument owners without probes', async () => {
  const cwd = resolve('test/jsbt-check/vectors/errors/zero-arg-no-warning');
  const res = await capture(() => runCli(['package.json'], { color: false, cwd }));
  deepStrictEqual(res, {
    ok: true,
    stderr: '',
...
should('errors does not probe optional internal error label parameters', async () => {
  const cwd = resolve('test/jsbt-check/vectors/errors/label-param');
  const res = await capture(() => runCli(['package.json'], { color: false, cwd }));
  deepStrictEqual(res, {
    ok: true,
    stderr: '',
...
should(
  ...
  async () => {
    const cwd = resolve('test/jsbt-check/vectors/errors/private-skip');
    const res = await capture(() => runCli(['package.json'], { color: false, cwd }));
    deepStrictEqual(res, {
      ok: true,
      stderr: '',
...
should('errors inspects objects returned from public function-valued properties', async () => {
  const cwd = resolve('test/jsbt-check/vectors/errors/function-output');
  const res = await capture(() => runCli(['package.json'], { color: false, cwd }));
  deepStrictEqual(res, {
    ok: true,
    stderr: '',
