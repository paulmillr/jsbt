...
should('bench-compare defaults to table output when colors are enabled', async () => {
  ...
  deepStrictEqual(
    lines.some((line) =>
      line.replace(/\x1b\[\d+(;\d+)*m/g, '').includes('compare  against first row')
    ),
    true
  );
  ...
  deepStrictEqual(
    lines.some((line) =>
      line.replace(/\x1b\[\d+(;\d+)*m/g, '').includes('name  js (from benchmark cases)')
    ),
    true
  );
  deepStrictEqual(
...
should('bench-compare highlights active filter and dimensions in table summary', async () => {
  ...
  deepStrictEqual(
    lines.some((line) => line.includes(`${'\x1b[34m'}name${'\x1b[0m'} x size`)),
    true
  );
  deepStrictEqual(
  ...
  deepStrictEqual(
    lines.some((line) => line.includes(`${'\x1b[34m'}JSBT_FILTER${'\x1b[0m'}`)),
    true
  );
  deepStrictEqual(
    lines.some((line) => line.includes(`${'\x1b[34m'}JSBT_BENCHMARK_DIMENSIONS${'\x1b[0m'}`)),
    true
  );
});
