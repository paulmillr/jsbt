...
import { cliArgs, createCachedSourceFile, emptyResult, loadTypeScriptApi, makeIssue, nodeStart, pickTSFiles, pkgTarget, relName, reportIssues, runSelf, usageText, walkAst, } from "./utils.js";
const usage = usageText('patterns', 'jsbt/patterns.ts');
const GLOBAL_VALUES = new Set(['Infinity', 'NaN', 'undefined']);
const loadTS = (pkgFile) => loadTypeScriptApi(pkgFile, 'TypeScript parser API', ['createSourceFile']);
const api = (ts) => ts;
const isKind = (ts, node, name) => !!node && node.kind === ts.SyntaxKind[name];
const isNode = (ts, node, name) => {
    if (!node)
        return false;
    const fn = api(ts)[`is${name}`];
    return typeof fn === 'function' ? fn(node) : isKind(ts, node, name);
};
const nameText = (ts, node) => isNode(ts, node, 'Identifier') ? node.text || '' : '';
const srcLine = (src, node) => {
    const pos = nodeStart(src, node);
    const loc = src.getLineAndCharacterOfPosition(pos);
    return { col: loc.character + 1, line: loc.line + 1 };
};
const emit = (out, src, node, level, kind, issue) => {
...
const sourceChild = (ts, node) => isNode(ts, node?.parent, 'SourceFile');
const spanMulti = (src, node) => {
    const start = nodeStart(src, node);
    const end = Math.max(start, node.end || start);
    return (src.getLineAndCharacterOfPosition(start).line !== src.getLineAndCharacterOfPosition(end).line);
};
const lineSpan = (src, node) => {
    const start = nodeStart(src, node);
    const end = Math.max(start, node.end || start);
    return (src.getLineAndCharacterOfPosition(end).line - src.getLineAndCharacterOfPosition(start).line + 1);
};
const lineOf = (src, node) => src.getLineAndCharacterOfPosition(nodeStart(src, node)).line;
const simpleValue = (ts, node) => isNode(ts, node, 'Identifier') ||
    isNode(ts, node, 'PropertyAccessExpression') ||
    isNode(ts, node, 'ElementAccessExpression');
const callName = (ts, node) => {
    if (!isNode(ts, node, 'CallExpression'))
        return '';
    const expr = node.expression;
    if (isNode(ts, expr, 'Identifier'))
        return expr.text || '';
    if (isNode(ts, expr, 'PropertyAccessExpression'))
        return expr.getText?.() || '';
    return '';
};
const paramNames = (ts, node) => {
    const names = [];
...
const returnCall = (ts, node) => {
    if (isNode(ts, node, 'ArrowFunction')) {
        if (isNode(ts, node.body, 'CallExpression'))
            return node.body;
        if (!isNode(ts, node.body, 'Block'))
            return;
    }
    const body = node.body;
    if (!body?.statements || body.statements.length !== 1)
        return;
...
const smallHelper = (ts, src, node) => {
    if (isNode(ts, node, 'ArrowFunction') && !isNode(ts, node.body, 'Block'))
        return true;
    const body = node.body;
    return !!body?.statements && body.statements.length <= 1 && lineSpan(src, node) <= 3;
};
const typeTarget = (ts, node) => {
    if (!isNode(ts, node, 'TypeReferenceNode') || node.typeArguments?.length)
        return '';
    const name = node.typeName;
    return isNode(ts, name, 'Identifier') ? name.text || '' : name?.getText?.() || '';
};
const constDecl = (ts, node) => {
    const flag = api(ts).NodeFlags?.Const || 0;
    const flags = node.parent?.flags || 0;
    return !!(flags & flag);
};
const collectImports = (ts, src) => {
    const out = new Set();
    walkAst(ts, src, (node) => {
...
const propertyIdentifier = (ts, node) => {
    const parent = node.parent;
    if (!parent)
        return false;
    if (isNode(ts, parent, 'PropertyAccessExpression') && parent.name === node)
        return true;
    return isNode(ts, parent, 'PropertyAssignment') && parent.name === node;
};
const refCounts = (ts, src) => {
    const refs = new Map();
    walkAst(ts, src, (node) => {
        if (!isNode(ts, node, 'Identifier'))
            return true;
        if (declIdentifier(ts, node) || propertyIdentifier(ts, node))
            return true;
...
const collectHelpers = (ts, src, issues) => {
    const out = [];
    walkAst(ts, src, (node) => {
        if (isNode(ts, node, 'FunctionDeclaration')) {
            ...
            if (pass) {
                emit(issues, src, node.name, 'error', 'wrapper', `helper only forwards its arguments to ${pass}`);
            }
        }
...
const checkUnusedExpr = (ts, src, node, issues) => {
    if (!isNode(ts, node, 'ExpressionStatement'))
        return;
    const expr = node.expression;
    if (isNode(ts, expr, 'VoidExpression') && simpleValue(ts, expr.expression)) {
        emit(issues, src, node, 'error', 'unused', 'do not silence unused values with void expression statement');
    }
    else if (simpleValue(ts, expr)) {
        emit(issues, src, node, 'error', 'unused', 'bare value expression statement does not use the value');
    }
};
const checkAliases = (ts, src, node, imports, issues) => {
    if (isNode(ts, node, 'VariableDeclaration')) {
        ...
        if (target) {
            emit(issues, src, node.name, 'error', 'alias', `pointless type alias; use ${target} directly`);
        }
    }
};
const checkBraces = (ts, src, node, issues) => {
    // TypeScript expression nodes exclude surrounding `if (` / `)`, so line-span the head too.
    const headMulti = (stmt) => lineOf(src, node) + 1 < lineOf(src, stmt);
    const needs = (stmt, cond) => !isNode(ts, stmt, 'Block') && (spanMulti(src, cond) || headMulti(stmt) || spanMulti(src, stmt));
    const anyMulti = (items) => items.some((item) => item && spanMulti(src, item));
    if (isNode(ts, node, 'IfStatement')) {
        ...
        if (!isNode(ts, node.statement, 'Block') &&
        ...
        if (needs(node.statement, node.expression)) {
            emit(issues, src, node, 'error', 'braces', 'multiline while condition or body must use braces');
        }
    }
};
const byPosition = (a, b) => a.line - b.line || a.col - b.col || a.issue.localeCompare(b.issue);
export const scanPatternText = (ts, file, text) => {
    ...
    for (const helper of helpers) {
        if (helper.exported || helper.passthrough || helper.topLevel || !helper.small)
            continue;
        if ((refs.get(helper.name) || 0) === 1) {
            emit(issues, src, helper.node, 'warning', 'helper', 'single-use helper; inline it or give it a real abstraction boundary');
        }
    }
    ...
    for (const file of pickTSFiles(target.cwd)) {
        ...
        for (const item of hits) {
            if (item.level === 'warning')
                ...
                result.failures++;
            issues.push(makeIssue(item.level === 'warning' ? 'warn' : 'error', relName(target.cwd, file), `${item.line}:${item.col}/${item.kind}`, item.issue, item.kind));
        }
    }
