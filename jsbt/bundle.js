...
import { bundleTempDir, npmInstall, rmBundleTempDir, writeBundleInput, writePkg, } from "../fs-modify.js";
import { camelParts, kb, readJson, runSelf } from "./utils.js";
const exec_ = promisify(exec);
const ex = (cmd) => exec_(cmd);
const usage = `usage:
...
const snakeToCamel = (snakeCased) => camelParts(snakeCased.split('-'));
// @namespace/ab-cd => namespace-ab-cd and namespaceAbCd
const getNames = (cwd) => {
    const curr = pjoin(cwd, 'package.json');
    let packageJsonName;
    try {
        packageJsonName = readJson(curr).name;
    }
    catch (error) {
...
const displayPath = (cwd, file) => {
    const full = resolve(file);
    const rel = relative(resolve(cwd), full);
    return rel && !/^\.\.(?:[/\\]|$)/.test(rel) && !isAbsolute(rel) ? rel : full;
};
const autoInput = (name) => `export * from '${name}';\n`;
const autoPackage = (cwd, name) => `${JSON.stringify({
    private: true,
    type: 'module',
    dependencies: { [name]: pathToFileURL(cwd).href },
}, undefined, 2)}\n`;
const ensureAutoFiles = (root, cwd, name) => {
    const pkg = pjoin(root, 'package.json');
    if (!existsSync(pkg))
        writePkg(pkg, autoPackage(cwd, name));
    const input = pjoin(root, 'input.js');
    if (!existsSync(input))
        writeBundleInput(input, autoInput(name));
};
const prepareAutoDir = (cwd) => {
...
const requireReportValue = (value, label) => {
    if (!value)
        throw new Error(`bundle report missing ${label}`);
    return value;
};
const bundleReportLines = (report, opts = {}) => [
    ...(opts.stats
        ? []
        : [
...
const runEsbuild = async (cwd, root, noPrefix, opts = {}) => {
    const names = getNames(cwd);
    const inp = `input.js`;
    ...
    try {
        ...
        for (const line of bundleReportLines(report, { stats: opts.stats }))
            console.log(line);
    }
    finally {
        process.chdir(prev);
...
const parseArgs = (argv, cwd = process.cwd()) => {
    ...
    for (let i = 0; i < argv.length; i++) {
        ...
        if (arg === '--dir') {
            const next = argv[++i];
            if (!next || next.startsWith('-') || directoryArg)
                usageErr();
            directoryArg = next;
        }
        else if (arg.startsWith('--dir=')) {
            const next = arg.slice('--dir='.length);
            if (!next || directoryArg)
                usageErr();
            directoryArg = next;
        }
...
export const runCli = async (argv, opts = {}) => {
    ...
    try {
        ...
        if (args.stats && target.temp)
            rmBundleTempDir(target.root);
    }
};
export const __TEST = {
    autoInput: autoInput,
