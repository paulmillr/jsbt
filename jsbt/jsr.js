...
import { cliArgs, dirEntries, emptyResult, importTypeText, literalText, loadTypeScriptApi, pkgTarget, readJson, readSource, recordIssue, relFile, reportIssues, resolveLocalImport, runSelf, sorted, tsSourceRel, usageText, walkAst, wantTSFile, } from "./utils.js";
const usage = usageText('jsr', 'check-jsr.ts');
const KEEP = new Set(['LICENSE', 'README.md', 'jsr.json']);
const norm = (file) => (file.startsWith('./') ? file : `./${file}`);
...
const jsrName = (name) => (name.startsWith('@') ? name : `@paulmillr/${name}`);
const parseJsr = (value) => {
...
const stringMap = (raw, map = (value) => value) => {
    const out = {};
...
const stringList = (raw) => Array.isArray(raw) ? raw.filter((item) => typeof item === 'string') : [];
const normalizeExports = (raw) => {
    if (typeof raw === 'string')
        return { '.': norm(raw) };
    return stringMap(raw, norm);
};
const GRAPH_FILE = /(?:\.d\.[cm]?ts|\.cts|\.mts|\.tsx|\.ts|\.cjs|\.mjs|\.jsx|\.js)$/;
const sourceRel = (cwd, dtsFile) => {
    const rel = tsSourceRel(norm(relative(cwd, dtsFile)));
    const src = norm(`src/${rel}`);
    const root = norm(rel);
    if (existsSync(resolve(cwd, src)))
        return src;
    if (existsSync(resolve(cwd, root)))
        return root;
    return src;
};
const resolveImportFile = (from, spec, want = wantTSFile) => resolveLocalImport(from, spec, {
...
const hasTS = (file) => {
    ...
    for (const ent of dirEntries(file)) {
        ...
        if (ent.isFile() && wantTSFile(cur))
            return true;
    }
    return false;
};
const pathCovers = (base, file) => base === file || (!!base && file.startsWith(`${base}/`));
const coveredBy = (inc, file) => {
    const cur = file.replace(/^\.\//, '');
    return inc.some((item) => pathCovers(item.replace(/^\.\//, ''), cur));
};
const importDeclTypeOnly = (ts, node) => {
...
const scanRefs = (ts, file) => {
    ...
    const add = (spec, typeOnly) => {
        if (!spec || spec.startsWith('node:'))
            return;
        if (spec.startsWith('.'))
            return void local.add(spec);
        const key = depRoot(spec);
        if (!key)
            return;
        if (!typeOnly || roots.get(key) === 'runtime')
            return void roots.set(key, 'runtime');
        if (!roots.has(key))
            roots.set(key, 'type');
    };
    walkAst(ts, source, (node) => {
        if (ts.isImportDeclaration(node))
            ...
            add(importTypeText(ts, node), true);
...
const graph = (ts, entries) => {
    ...
    while (stack.length) {
        ...
        if (graphFiles.has(file) || !existsSync(file))
            continue;
        graphFiles.add(file);
        if (wantTSFile(file))
            files.add(file);
        ...
        for (const [root, mode] of refs.roots) {
            if (mode === 'runtime' || roots.get(root) === 'runtime')
                roots.set(root, 'runtime');
            else if (!roots.has(root))
                roots.set(root, 'type');
        }
    }
...
const hasRuntimePkgDep = (raw, key) => typeof raw.dependencies?.[key] === 'string' ||
    typeof raw.optionalDependencies?.[key] === 'string' ||
    typeof raw.peerDependencies?.[key] === 'string';
const depSpec = (raw, key) => {
    for (const set of [
        raw.dependencies,
...
const alreadyUnignored = (exclude, rel) => {
    for (const item of exclude) {
        ...
        if (!raw || /[*?[\\]/.test(raw))
            continue;
        if (pathCovers(raw, rel))
            return true;
    }
    return false;
};
const ignoredGraphFiles = (cwd, files) => {
    if (!files.length)
        return new Map();
...
export const runCli = async (argv, opts = {}) => {
    ...
    const issue = (file, sym, text, kind) => {
    ...
    if (!existsSync(jsrFile)) {
        ...
        const rawPkg = readJson(pkgFile);
        const rawJsr = readJson(jsrFile);
        ...
        for (const key of sorted(Object.keys(exp))) {
            ...
            if (got === want)
                continue;
            issue('jsr.json', 'exports', `${got ? 'fix' : 'missing'} jsr export mapping; use ${key} -> ${want}`, 'jsr-export');
        }
        for (const key of sorted(Object.keys(act))) {
            if (exp[key])
                continue;
            issue('jsr.json', 'exports', `remove unexpected jsr export mapping; drop ${key} -> ${act[key]}`, 'jsr-export-extra');
        }
        const wantName = jsrName(pkg.name);
        if (rawJsr.name !== wantName) {
            issue('jsr.json', 'name', `name mismatch; expected ${wantName} from package.json`, 'jsr-name');
        }
        if (rawJsr.version !== rawPkg.version) {
            issue('jsr.json', 'version', `version mismatch; expected ${String(rawPkg.version || '')} from package.json`, 'jsr-version');
        }
        const ts = (opts.loadTS || loadTS)(pkgFile);
        const src = graph(ts, Object.values(exp)
        ...
        for (const key of src.runtimeRoots) {
            if (!hasRuntimePkgDep(rawPkg, key)) {
                issue('package.json', 'dependencies', `add package dependency for exported source import ${key}`, 'jsr-dep');
            }
        }
        for (const key of src.roots) {
            ...
            if (got.name === want && (!wantVer || got.version === wantVer))
                continue;
            issue('jsr.json', 'imports', `fix jsr import mapping; use ${key} -> ${wantImport}`, 'jsr-import');
        }
        for (const key of sorted(Object.keys(jsrImports))) {
            if (src.roots.includes(key))
                continue;
            issue('jsr.json', 'imports', `remove unexpected jsr import mapping; drop ${key} -> ${jsrImports[key]}`, 'jsr-import-extra');
        }
        const include = stringList(rawJsr.publish?.include);
        for (const item of ['LICENSE', 'README.md', 'jsr.json']) {
            if (!include.includes(item)) {
                issue('jsr.json', 'publish', `add required publish entry; use ${item}`, 'jsr-publish-required');
            }
        }
        for (const item of include) {
            if (KEEP.has(item))
                continue;
            if (hasTS(resolve(cwd, item)))
                continue;
            issue('jsr.json', 'publish', `remove non-source publish entry; drop ${item}`, 'jsr-publish-source');
        }
        for (const file of sorted(src.files.map((item) => relFile(cwd, item)))) {
            if (!coveredBy(include, file)) {
                issue('jsr.json', 'publish', `add publish coverage for exported source graph; use ${file}`, 'jsr-publish');
            }
        }
    }
