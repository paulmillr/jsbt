 ...
 */
import { publicCtx, publicRows } from "./public.js";
import { cliArgs, color, emptyResult, fileUrl, paint, recordIssue, reportIssues, runSelf, runWorker, skipRootImportTrap, stripAnsi, table, usageText, } from "./utils.js";
...
const measureOnce = (spec) => runWorker(WORKER, {
    data: { spec },
    error: (error) => ({ error }),
});
const measure = async (jsFile) => {
...
const fmt = (ms) => ms.toFixed(2);
const rowText = (item, colorOn) => {
...
const slowText = (item) => `${fmt(item.ms || 0)}ms (x${item.ratio.toFixed(2)} from baseline)`;
export const runCli = async (argv, opts = {}) => {
    ...
    const rows = await publicRows(ctx, async (mod) => ({
        limit: Infinity,
        ratio: 1,
        warn: false,
        ...(await measure(mod.jsFile)),
    }));
    const okRows = rows.filter((item) => typeof item.ms === 'number');
    ...
    if (!opts.quiet) {
        ...
        const sizes = headers.map((head, i) => Math.max(head.length, ...values.map((row) => stripAnsi(row[i]).length)));
        const print = table(console.log);
        print.drawHeader(sizes, headers);
        let prev;
        for (const row of values)
            prev = print.printRow(row, prev, sizes, ['module']);
    }
    const logs = [];
