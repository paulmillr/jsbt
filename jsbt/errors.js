...
import { publicCtx, publicEntries } from "./public.js";
import { cliArgs, color, docCommentLines, emptyResult, fileUrl, ident, jsbtWorkerLimit, lineIndex, loadTypeScriptApi, nodeStart, nodeText, paint, printIssues, readText, recordIssue, relName, runSelf, runWorker, status, summary, tsSourceRel, usageText, walkAst, withTempFile, } from "./utils.js";
const usage = usageText('errors', 'check-errors.ts');
const TIMEOUT = 120_000;
const MAX_PROBES_PER_ARG = 12;
const defaultProbeLimit = () => Math.max(1, Math.min(availableParallelism?.() || 4, 16));
const loadTs = (pkgFile) => {
    return loadTypeScriptApi(pkgFile, 'TypeScript compiler API', ['createSourceFile']);
};
const sourceFile = (cwd, jsRel) => {
...
const runtimeUrl = (cwd, jsRel) => {
    const js = resolve(cwd, jsRel);
    return fileUrl(existsSync(js) ? js : sourceFile(cwd, jsRel));
};
const runtimeEntries = (ctx) => publicEntries(ctx).map(({ jsRel, spec }) => ({
...
const privateName = (name) => name.startsWith('_');
const publicName = (name) => ident(name) && !privateName(name);
...
const publicSources = (entries) => {
    const out = new Map();
    for (const item of entries)
        if (existsSync(item.file) && !privateName(basename(item.file)))
            out.set(item.file, item);
    return [...out.values()].sort((a, b) => a.file.localeCompare(b.file));
};
const specMap = (entries) => {
    const out = new Map();
    for (const item of entries)
        out.set(item.spec, item.url);
    return out;
};
const rewriteImports = (specs, code) => {
    return code.replace(/(\bfrom\s*['"]|\bimport\s*\(\s*['"])([^'"]+)(['"])/g, (all, head, spec, tail) => {
        const next = specs.get(spec);
        return next ? head + next + tail : all;
    });
};
const cleanDoc = (raw) => docCommentLines(raw, false).join('\n');
const paramDocs = (docs) => {
    const out = [];
    for (const match of docs.matchAll(/@param\s+(?:\{[^}]*\}\s*)?([A-Za-z_$][\w$]*)/g)) {
        const name = match[1];
        if (!out.some((param) => param.name === name))
            out.push({ name, optional: false });
    }
    return out;
};
const splitParams = (raw) => raw
    .split(',')
    .map((part) => {
    const optional = /^\s*\.\.\./.test(part) || /=/.test(part) || /^[\s.]*[A-Za-z_$][\w$]*\?/.test(part);
    const name = part
        .replace(/=.*$/s, '')
        .replace(/^\s*\.\.\./, '')
        .replace(/[:?].*$/s, '')
...
const exportedLocals = (text) => {
    ...
    for (const match of text.matchAll(/export\s*\{([^}]*)\}/g)) {
        for (const raw of match[1].split(',')) {
            ...
            if (publicName(name) && publicName(pub))
                out.set(name, pub);
        }
    }
    return out;
};
const ownerMatch = (exported, match, params, callable = true, generic = false) => {
    const local = match[2];
...
const paramList = (parsed, docs) => (parsed.length ? parsed : docs);
const constParams = (next) => {
    ...
    const direct = body.match(new RegExp('^\\s*(?:\\/\\*[\\s\\S]*?\\*\\/\\s*)*(?:async\\s+)?' +
        `(?:${arrow}|function\\s*\\(([^)]*)\\))`));
    return direct ? splitParams(direct[1] || direct[2] || direct[3] || '') : undefined;
};
const ownerInfo = (text, pos, docs, exported) => {
    ...
    const fn = next.match(/^\s*(export\s+)?(?:async\s+)?function\s+([A-Za-z_$][\w$]*)\s*(<[\s\S]*?>)?\s*\(([^)]*)\)/);
    if (fn) {
    ...
    if (cls)
        return ownerMatch(exported, cls, params);
    const typ = next.match(/^\s*(export\s+)?(?:type|interface)\s+([A-Za-z_$][\w$]*)/);
    if (typ)
        return ownerMatch(exported, typ, params, !!params.length);
    const cnst = next.match(/^\s*(export\s+)?(?:const|let|var)\s+([A-Za-z_$][\w$]*)/);
    if (!cnst)
        return undefined;
    const parsed = constParams(next);
    return ownerMatch(exported, cnst, parsed?.length ? parsed : params, parsed !== undefined || !!params.length);
};
const exampleRows = (src) => {
    const { file, url } = src;
...
const importedNames = (ts, sf, pkg) => {
    ...
    for (const stmt of sf.statements || []) {
        ...
        if (sub && privateName(basename(sub)))
            continue;
        const clause = stmt.importClause;
        if (clause?.name?.text && publicName(clause.name.text))
        ...
        if (named && ts.isNamedImports?.(named)) {
            for (const el of named.elements || []) {
                ...
                if (publicName(local) && publicName(imported))
                    names.add(local);
            }
        }
    }
...
const rootName = (ts, node) => {
    ...
    if (base)
        return rootName(ts, base);
    return '';
};
const memberExpr = (ts, node) => ts.isPropertyAccessExpression(node) || !!ts.isElementAccessExpression?.(node);
const memberBase = (ts, node) => memberExpr(ts, node) ? node.expression : undefined;
const propName = (ts, node, literalOnly = false) => {
    if (ts.isPropertyAccessExpression(node))
        return node.name?.text || '';
    if (!ts.isElementAccessExpression?.(node))
        return '';
    const arg = node.argumentExpression;
    if (literalOnly && !ts.isStringLiteral?.(arg))
        return '';
    return literalOnly ? arg.text || '' : arg?.getText?.() || '';
};
const finalName = (ts, node) => {
    if (!node)
        return '';
    if (ts.isIdentifier(node))
        return node.text || '';
...
const publicRoot = (root, imports, implicit) => !privateName(root) && (imports.has(root) || (!!implicit && root === implicit));
const publicExpr = (ts, node, imports, implicit) => {
    if (!node)
        return false;
    return publicRoot(rootName(ts, callTarget(ts, node)), imports, implicit);
};
const callTarget = (ts, node) => ts.isCallExpression(node) || ts.isNewExpression?.(node) ? node.expression : node;
const selfName = (ts, node, sf) => {
    const base = memberBase(ts, node);
...
const probeParam = (param) => !privateName(param.name) && !INTERNAL_LABEL.test(param.name);
const deepArg = (arg, name, generic) => !generic &&
    (/^\s*[{[]/.test(arg) || /\b(?:opts?|options?|params?|config|settings)\b/i.test(name || arg));
const argsMeta = (params, args, generic = false) => {
    if (params.length) {
        return {
...
const addMethod = (out, path, params) => {
    ...
    const add = (key) => {
        ...
        if (!prev || params.length > prev.length)
            out.set(key, params);
    };
    add(path);
...
const privateMember = (ts, member, name) => privateName(name) ||
    member.name?.kind === ts.SyntaxKind.PrivateIdentifier ||
    (member.modifiers || []).some((mod) => mod.kind === ts.SyntaxKind.PrivateKeyword);
const methodParams = (ts, sources) => {
    const out = new Map();
    const hidden = new Set();
    const hiddenClasses = new Set();
    const readType = (sf, node, path = '') => {
        if (!node)
            return;
        if (ts.isParenthesizedTypeNode?.(node))
            return readType(sf, node.type, path);
        if (ts.isFunctionTypeNode?.(node))
            return addMethod(out, path, astParams(sf, node.parameters));
        if (ts.isTypeLiteralNode?.(node))
            return readMembers(sf, node.members || [], path);
        if (Array.isArray(node.types))
            for (const item of node.types)
                readType(sf, item, path);
    };
    const readMembers = (sf, members = [], prefix = '') => {
        for (const member of members) {
            ...
            if (ts.isMethodSignature?.(member))
                addMethod(out, path, astParams(sf, member.parameters));
            else if (ts.isPropertySignature?.(member))
                readType(sf, member.type, path);
        }
    };
    const readClass = (sf, node) => {
...
const calls = (ts, ex, pkg, ownerMap, methods) => {
    ...
    const publicVarOwners = new Map();
    const seen = new Set();
    const privateMethodCall = (root, last, methodPath) => {
        if (privateName(last))
            return true;
        if (!methodPath)
            return false;
        if (methods.private.has(methodPath))
            return true;
        const owner = publicVarOwners.get(root);
        return !!owner && methods.private.has(`${owner}.${last}`);
    };
    const publicMethod = (expr) => {
    ...
    const add = (node, expr, argsRaw, newExpr = false) => {
        const args = [...(argsRaw || [])].map((arg) => arg.getText(sf));
        ...
        if ((!imports.has(root) && !needsImport && !method) || seen.has(text))
            return false;
        const last = finalName(ts, expr);
        if (privateName(root) || privateName(last))
            return false;
        ...
        if (method && privateMethodCall(root, last, methodPath))
            return false;
        const { argNames, generic, missing, probe } = callArgs(ownerMap, methods, ex, last, direct, methodPath, args);
        ...
        const name = method && displayOwner && methodPath
    ...
    walkAst(ts, sf, (node) => {
        ...
        if (ts.isCallExpression(node) && add(node, node.expression, node.arguments))
            return false;
...
const ownerMap = (rows) => {
    ...
    for (const item of rows)
        if (item.owner)
            out.set(item.owner.name, item.owner);
    return out;
};
const workRows = (ctx, ts, entries) => {
...
const instrumentedCode = (work) => {
    ...
    for (const chunk of chunks)
        out = out.slice(0, chunk.start) + chunk.text + out.slice(chunk.end);
    return out;
};
const harnessCode = (specs, work) => {
...
`;
const probe = async (specs, work, runDir, timeoutMs) => {
    return withTempFile(runDir, { code: harness(work, harnessCode(specs, work)), ext: 'ts', prefix: '.__errors-check-' }, async (file) => {
        ...
        return await runWorker(workerCode, {
...
const runProbeLimit = async (ctx, specs, items, limit, runDir, timeoutMs) => {
...
const auditLines = (items, colorOn) => {
    ...
    for (const item of items) {
        ...
        if (prev) {
            if (!prev.some((old) => old.name === row.name && old.msg === row.msg))
                prev.push(row);
        }
        else
            groups.set(key, [row]);
...
const printAudit = (items, colorOn) => {
    for (const line of auditLines(items, colorOn))
        console.log(line);
};
const finish = (res, colorOn) => {
    if (res.failures) {
        console.error(`${status('error', colorOn)} summary: ${summary(res)}`);
        throw new Error('Errors check found issues');
    }
    if (res.warnings)
        return console.error(`${status('warn', colorOn)} summary: ${summary(res)}`);
...
export const runCli = async (argv, opts = {}) => {
    ...
    const audit = [];
    for (const item of rows) {
        if (item.calls.some((call) => call.ownerCall))
            continue;
        if (!item.owner?.callable)
            continue;
        // Zero-argument ownerMap such as removed-alias stubs cannot produce wrong-argument probes.
        if (!item.owner.params.some(probeParam))
            continue;
        recordIssue(out, logs, 'WARN', relName(ctx.cwd, item.file), `${item.line}/example`, 'could not derive valid runtime probes from TSDoc example', 'errors-example');
    }
    if (!rows.length) {
        recordIssue(out, logs, 'INFO', 'package.json', 'examples', 'no public callable TSDoc examples found', 'errors-example');
    }
    if (opts.examplesOnly) {
        printIssues('errors', logs, colorOn);
        return finish(out, colorOn);
    }
    const runs = await runProbeLimit(ctx, specs, items, opts.limit || jsbtWorkerLimit(defaultProbeLimit()), runDir, opts.timeoutMs || TIMEOUT);
    for (const { item, rel, res } of runs) {
        if (res.error) {
            recordIssue(out, logs, 'WARN', rel, `${item.line}/example`, 'example probe failed: ' + res.error, 'example');
            continue;
        }
        if (!res.issues.length)
            out.passed += res.probed || 1;
        ...
        for (const issue of res.issues) {
            recordIssue(out, logs, issue.level, rel, `${issue.line}/${issue.call}`, issue.detail, `errors-${issue.kind}`);
        }
    }
    printIssues('errors', logs, colorOn);
