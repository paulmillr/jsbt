...
import { cliArgs, emptyResult, pkgTarget, recordIssue, relFile, reportIssues, runSelf, textLines, usageText, } from "./utils.js";
const usage = usageText('jsrpublish', 'jsbt/jsrpublish.ts');
const SNIP = 8;
const probeText = (probe) => [probe.stdout, probe.stderr, probe.error].filter(Boolean).join('\n');
const runPublish = (cwd, allowSlowTypes) => {
    const args = [
        'publish',
...
const relevantLines = (text) => {
    ...
    if (!lines.length)
        return [];
    const start = lines.findIndex((line) => /^(?:TS\d+ \[(?:ERROR|WARNING)\]:|(?:error|warning)\[[^\]]+\]:|error: )/.test(line));
    if (start !== -1)
        return lines.slice(start);
    return lines.filter((line) => !line.startsWith('Check ') &&
        !line.startsWith('Checking for slow types') &&
        !line.startsWith('Simulating publish') &&
        !line.startsWith('Found '));
};
const preview = (probe, full) => {
    const lines = relevantLines(probeText(probe));
    if (full || lines.length <= SNIP)
        return lines;
    return [...lines.slice(0, SNIP), '...'];
};
const recordPublishIssue = (out, issues, level, kind, text, probe, cwd, full) => {
    const lines = relevantLines(probeText(probe));
    const loc = lines
...
export const runCli = async (argv, opts = {}) => {
    ...
    if (strict.skipped) {
        recordIssue(out, issues, 'info', 'jsr.json', 'publish', `deno publish dry-run skipped; ${strict.skipped}`, 'jsrpublish-skip');
    }
        ...
        if (!slow.status && !slow.error) {
            ...
            recordPublishIssue(out, issues, 'WARN', 'jsrpublish-slow', [
                'deno publish fails without --allow-slow-types;',
                `rerun passes with --allow-slow-types${hint}`,
            ].join(' '), strict, cwd, full);
        }
        else {
            const hint = full ? '' : '; see npm run check jsrpublish for full output';
            recordPublishIssue(out, issues, 'ERROR', 'jsrpublish', `deno publish fails even with --allow-slow-types${hint}`, probeText(slow) ? slow : strict, cwd, full);
        }
    }
