...
import { ident, pkgTarget, readText, relName, runSelf } from 'baler';
import {
  cliArgs,
  emptyResult,
  loadTypeScriptApi,
...
// Canonical-helper-block guidance (bytes-helper) is suppressed for now; the wrap
// findings themselves still report. Flip to true to restore the guidance.
const HELPER_GUIDANCE = false;
const checkHelpers = (state: State, files: FileCtx[]) => {
  if (!HELPER_GUIDANCE) return;
