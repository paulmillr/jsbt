...
import { assertTemp, installRunDeps, rm, rmTempDir, tempDir, write } from '../fs-modify.ts';

import { wantColor } from '../env.ts';
import { color, loadNear, paint, pkgTarget } from '../jsbt.ts';
...
// machine-level esbuild cache; npm only on cold cache). No test/build involvement.
export const prepareRunDir = (cwd: string, name: string, dir: string): string => {
  assertTemp(dir);
  installRunDeps(cwd, name, dir);
  return dir;
};
