// Shared helpers for the jsbt-check binary's audit modules. The slim jsbt binary
// never imports this file; the dependency direction is jsbt-check -> jsbt only.
...
import { assertTemp, installRunDeps, rm, rmTempDir, tempDir, write } from 'baler/fs-modify.js';

import { color, paint, wantColor } from 'baler/env.js';
import { err, loadModuleApi, loadNear, pkgTarget, readJson, readText } from 'baler';
...
export type TableApi = {
  drawBottom: (sizes: number[]) => void;
  drawTop: (sizes: number[]) => void;
...
export const loadTypeScriptApi = <T>(pkgFile: string, api: string, keys: readonly string[]): T => {
  try {
    return loadModuleApi<T>(pkgFile, 'typescript', api, keys);
  } catch (error) {
    // typescript@7 (the Go compiler) removed the JS compiler API: its main export is
    // just a version string, so the shape check fails. Name the fix, not the symptom.
    if (/^expected /.test((error as Error).message)) {
      const version = (() => {
        try {
          return (createRequire(pkgFile)('typescript/package.json') as { version?: string })
            .version;
        } catch {
          return undefined;
        }
      ...
      if (version && parseInt(version, 10) >= 7)
        throw new Error(
          `typescript@${version} does not ship the ${api}; ` +
            `install typescript@6 in the target repo (npm i -D typescript@6)`
        );
    }
    throw error;
  }
};
export const loadTs = (pkgFile: string): TsCheck =>
  loadTypeScriptApi<TsCheck>(pkgFile, 'TypeScript compiler API', ['createProgram']);
...
export const table = (log: (line: string) => void): TableApi => {
  ...
  // Opening/closing borders: `┬`/`┴` junctions instead of `┼`, so the table reads as framed.
  const drawTop = (sizes: number[]) => log(sizes.map((size) => CH.repeat(size + 1)).join(TP));
  const drawBottom = (sizes: number[]) => log(sizes.map((size) => CH.repeat(size + 1)).join(BT));
  const printRow = (
    values: string[],
    prev: string[] | undefined,
    sizes: number[],
  ...
  return { drawBottom, drawHeader, drawSeparator, drawTop, printRow };
};
...
// The one sanctioned user-repo write: `jsbt-check size --generate-jsbtrc` produces or
// updates the committed config. The filename is fixed here, never caller-supplied.
...
// Prepares the isolated run dir: manifest plus node_modules (symlink-assembled from the
// machine-level esbuild cache and the project's own installed deps; npm only on cold
// cache). No test/build involvement.
...
export const formatIssue = (level: Level, head: string, ref: Ref, on: boolean): string =>
  `${tag(level, on)} ${head}: ${refLoc(ref)} ${ref.issue}`;
...
// Symbol-less refs (e.g. size budget selectors) are already a complete location.
const refLoc = (ref: Ref): string => (ref.sym ? `${ref.file}:${ref.sym}` : ref.file);
