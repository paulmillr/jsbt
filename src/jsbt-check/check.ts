...
import { loadTs, runSize, type Built } from '../jsbt.ts';
import { runCli as runTypeImport } from './typeimport.ts';
import { color, err, paint, wantColor, withSourceFileCache } from '../jsbt.ts';
import {
  defaultFast,
  fastWorkerCount,
  formatIssue,
...
const runSizeCheck = async (opts: {
  ...
  await runSize({
    ...
    outDir: opts.outDir,
    // Size stats are `jsbt size`'s job; the check only audits the built bundles.
    silent: true,
  });
  const ts = loadTs(`${opts.cwd ?? process.cwd()}/package.json`) as unknown as TsLike;
  const issues = audit(
    ts,
  ...
  for (const entry of built) {
    ...
    for (const item of list) {
      const issue: TreeIssue = { file: entry.name, id: entry.id, line: item.line, text: item.text };
      opts.onIssue?.(issue);
...
const checkArgs = (argv: string[]) => {
  ...
  if (head) return { head, help: false, outArg: CHECK_OUT, pkgArg: 'package.json' };
  ...
  return { head: undefined, help: false, outArg: CHECK_OUT, pkgArg: 'package.json' };
};
const checkTasks = {
