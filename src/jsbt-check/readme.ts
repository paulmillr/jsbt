...
Like `baler bundle`, it runs `npm install` in the selected run/build directory before checking.
...
dir installing the checked package as a `file:` dependency.
 */
import { isBuiltin } from 'node:module';
import { basename, resolve } from 'node:path';
import { wantColor } from 'baler/env.js';
import { err, guardChild, readJson, readText, runSelf } from 'baler';
import {
  compact,
  emptyResult,
  execText,
...
// Bare package names imported by example code — the raw material for generating the
// `.jsbtrc.json` exampleDependencies allowlist. Relative/absolute specifiers and node
// builtins are not installable dependencies; skip them.
const IMPORT_SPEC = /(?:from\s*|import\s*\(?\s*|require\s*\(\s*)['"]([^'"\n]+)['"]/g;
const collectImportNames = (code: string, names: Set<string>): void => {
...
export const readmeImportNames = (text: string): string[] => {
  const names = new Set<string>();
  for (const block of parseReadme(text))
    if (block.runnable && block.kind) collectImportNames(block.code, names);
  return [...names].sort();
};
// TSDoc @example fences live inside `/** */` comments with `* `-framed lines: strip
// the frame, then harvest fence imports the same way README fences are harvested.
const DOC_COMMENT = /\/\*\*([\s\S]*?)\*\//g;
const DOC_FENCE = /```[^\n]*\n([\s\S]*?)```/g;
export const docExampleImportNames = (text: string): string[] => {
  const names = new Set<string>();
  for (const comment of text.matchAll(DOC_COMMENT)) {
    const body = comment[1]
      .split('\n')
      .map((line) => line.replace(/^\s*\* ?/, ''))
      .join('\n');
    for (const fence of body.matchAll(DOC_FENCE)) collectImportNames(fence[1], names);
  }
  return [...names].sort();
};
