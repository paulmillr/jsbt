...
// jsbt's env namespace stays JSBT_*: baler's csvEnabled rule, jsbt's own knob.
// Off outside a CLI process (browsers), like csvEnabled.
const csvOn = cliProcess() ? envFlag(cliProcess()?.env?.JSBT_CSV) || !colorOn : false;
